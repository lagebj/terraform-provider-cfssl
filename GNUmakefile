...
testacc: fmtcheck
	TF_ACC=1 go test $(TEST) -v $(TESTARGS) -timeout $(TESTTIMEOUT) -ldflags="-X=github.com/lagebj/terraform-provider-cfssl/version.ProviderVersion=acc"

acctests: fmtcheck
	TF_ACC=1 go test -v ./internal/services/$(SERVICE) $(TESTARGS) -timeout $(TESTTIMEOUT) -ldflags="-X=github.com/lagebj/terraform-provider-cfssl/version.ProviderVersion=acc"
...
website-lint:
	...
	@tfproviderdocs check -provider-name=cfssl -require-resource-subcategory \
